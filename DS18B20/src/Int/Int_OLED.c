...
void Int_OLED_WriteData(u8 bytes[], u8 len)
{
    ...
    Dri_IIC_ReceiveAck();
    Dri_IIC_SendByte(DATA_MODE); // 连续的data
    Dri_IIC_ReceiveAck();
...
void Int_OLED_WriteCmd(u8 bytes[], u8 len)
{
    ...
    Dri_IIC_SendByte(DEV_ADDR);
    Dri_IIC_ReceiveAck();
    Dri_IIC_SendByte(CMD_MODE); // 连续的命令
    Dri_IIC_ReceiveAck();
...
void Int_OLED_SetPoint(u8 x, u8 y)
{
    u8 cmd_bytes[3];
    // 页位置(y)
    cmd_bytes[0] = 0xB0 | y;
